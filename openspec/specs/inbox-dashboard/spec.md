# inbox-dashboard Specification

## Purpose

The inbox-dashboard capability surfaces DRAFT notes in the Dashboard so users have visibility and control over the AI processing queue. The "📥 Inbox" section lists every draft owned by the current user, updates in real time as new captures arrive, and gives the user two ways to trigger processing: per-card ("Procesar con IA") and global ("✨ Procesar todo"). Each card exposes idle/loading/success/error states, never deletes data on error, and offers a retry path. The component is written in strict TypeScript with no `any` and no cascading renders from synchronous `setState` inside `useEffect`.
