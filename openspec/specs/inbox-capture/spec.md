...
The inbox-capture capability lets authenticated users save thoughts to the system instantly, with no waiting on AI. A capture creates a Note in `DRAFT` status and a placeholder domain, returns within 300ms, and dispatches a `zf:draft` event so the Dashboard Inbox can pick it up. Capture never invokes the AI layer — classification, embeddings, and relationships are deferred to inbox-processing, which runs only when the user chooses to process a draft. This separation guarantees the capture modal never freezes and the user never loses a thought to a slow or failing AI call.
